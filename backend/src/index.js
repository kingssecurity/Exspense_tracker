...
app.put('/api/users/password', (req, res) => {
  ...
  if (newPassword.length < 4) return res.status(400).json({ error: 'كلمة المرور قصيرة' });
...
app.get('/api/summary', (req, res) => {
  ...
  transactions.filter(t => t.type === 'expense').forEach(t => {
    ...
    breakdown[t.category].total += t.amount || 0; breakdown[t.category].count++;
  });
...
for (const p of possiblePaths) {
  if (fs.existsSync(p) && fs.existsSync(path.join(p, 'index.html'))) { frontendPath = p; break; }
}
if (frontendPath) {
